...
.PHONY: build
build:
	docker buildx build --platform linux/s390x --build-arg "TARGETPLATFORM=linux/s390x" -t $(ELECTOR_REPOSITORY):s390x-$(ELECTOR_VERSION) .
	docker buildx build --platform linux/amd64 --build-arg "TARGETPLATFORM=linux/amd64" -t $(ELECTOR_REPOSITORY):amd64-$(ELECTOR_VERSION) .
	docker buildx build --platform linux/arm64 --build-arg "TARGETPLATFORM=linux/arm64" -t $(ELECTOR_REPOSITORY):arm64-$(ELECTOR_VERSION) .
